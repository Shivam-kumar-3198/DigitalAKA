...
const EMPTY: Omit<Service, "id" | "createdAt"> = {
  title: "", description: "", iconName: "Zap", active: true, order: 0,
};

const ICONS = [
  "Palette", "CodeXml", "LineChart", "Sparkles", "Megaphone", "Wrench",
  "Mail", "MessageSquare", "Globe", "ShieldCheck", "Zap", "Star",
  "TrendingUp", "BarChart2", "Users", "Briefcase", "Settings", "Cloud",
];
...
function Modal({
  ...
  onClose: () => void;
  onSave: (data: Omit<Service, "id" | "createdAt">) => Promise<void>;
}) {
      ...
      <div className="relative w-full max-w-lg rounded-2xl bg-white shadow-2xl">
        ...
        <form onSubmit={handleSubmit} className="space-y-4 p-6">
          <div>
            ...
            <input name="title" required value={form.title} onChange={handleChange} placeholder="e.g. Web Development" className={inputCls} />
          </div>

          <div>
            <label className={labelCls}>Description *</label>
            <textarea name="description" required rows={3} value={form.description} onChange={handleChange} placeholder="Describe what this service offers…" className={inputCls + " resize-none"} />
          </div>

          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className={labelCls}>Icon Name</label>
              <select name="iconName" value={form.iconName} onChange={handleChange} className={inputCls + " cursor-pointer"}>
                {ICONS.map((i) => <option key={i}>{i}</option>)}
              </select>
              <p className="mt-1 text-[10px] text-slate-400">Lucide icon name</p>
            </div>
            <div>
...
export default function ServicesPage() {
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(q, (snap) => {
      setItems(snap.docs.map((d) => ({ id: d.id, ...d.data() } as Service)));
      setLoading(false);
    });
    return unsub;
  }, []);

  const save = useCallback(async (data: Omit<Service, "id" | "createdAt">) => {
    if ((modal as Service)?.id) {
      await updateDoc(doc(db, "services", (modal as Service).id), data);
    } else {
  ...
  const toggleActive = useCallback(async (item: Service) => {
    await updateDoc(doc(db, "services", item.id), { active: !item.active });
  }, []);
        ...
        <div className="rounded-2xl border border-slate-100 bg-white shadow-sm overflow-hidden">
          {loading ? (
            ...
            <div className="flex flex-col items-center gap-4 py-16 text-center">
              <Zap className="h-12 w-12 text-slate-200" strokeWidth={1.5} />
              <div>
                <p className="font-semibold text-slate-600">No services yet</p>
                <p className="mt-1 text-sm text-slate-400">Define the services you offer to clients.</p>
              </div>
              <button
                onClick={() => setModal({})}
              ...
              {items.map((item) => (
                ...
                >
                  ...
                  <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-[#1d5cf5]/10">
                    <Zap className="h-5 w-5 text-[#1d5cf5]" strokeWidth={1.8} />
                  </div>

                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap">
                      <p className="font-semibold text-slate-900">{item.title}</p>
                      <span className="text-xs text-slate-400 font-medium">· {item.iconName}</span>
                      {item.active ? (
