...
export default function AdminDashboard() {
  const { user } = useAuth();
  const [stats, setStats]           = useState<Stats>({ contacts: 0, unread: 0, faqs: 0, testimonials: 0, services: 0 });
  ...
  async function fetchData() {
    try {
      const [contactsSnap, unreadSnap, faqsSnap, testimonialsSnap, servicesSnap, recentSnap] =
        await Promise.all([
      ...
      // Collections may be empty on first run — that's fine
    } finally {
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-[#1d5cf5] to-blue-700 p-6 text-white shadow-[0_8px_30px_rgba(29,92,245,0.3)]">
        ...
        <div className="pointer-events-none absolute bottom-0 left-1/2 h-32 w-64 -translate-y-0 rounded-full bg-blue-800/40 blur-2xl" />
      ...
      <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
        {STAT_CARDS(stats).map(({ label, value, icon: Icon, color, bg, href, badge }) => (
          <Link
            key={label}
            href={href}
          ...
          >
            ...
            <ArrowRight className="absolute right-4 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-300 opacity-0 transition-all group-hover:opacity-100 group-hover:translate-x-0.5" />
          </Link>
        ))}
      </div>
      ...
      <div className="rounded-2xl border border-slate-100 bg-white shadow-sm">
        ...
        {loadingStats ? (
          ...
          <div className="flex flex-col items-center gap-3 py-12 text-center">
            ...
            <p className="text-xs text-slate-300">They'll appear here once visitors submit the form.</p>
          </div>
        ) : (
          <div className="divide-y divide-slate-50">
            {recent.map((c) => (
      ...
      <div>
        ...
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
          {[
            { href: "/admin/contacts",     label: "View Contacts",      icon: Mail,               color: "text-[#1d5cf5]",  bg: "bg-[#1d5cf5]/8" },
            { href: "/admin/faqs",         label: "Add FAQ",            icon: HelpCircle,          color: "text-violet-600", bg: "bg-violet-50" },
            { href: "/admin/testimonials", label: "Add Testimonial",    icon: MessageSquareQuote,  color: "text-emerald-600", bg: "bg-emerald-50" },
            { href: "/admin/services",     label: "Manage Services",    icon: Zap,                 color: "text-amber-600",  bg: "bg-amber-50" },
          ].map(({ href, label, icon: Icon, color, bg }) => (
            <Link
              key={href}
              href={href}
              className="flex flex-col items-center gap-2.5 rounded-2xl border border-slate-100 bg-white p-5 text-center shadow-sm transition hover:-translate-y-0.5 hover:shadow-md"
            >
