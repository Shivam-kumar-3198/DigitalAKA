...
const OVERVIEW_CARDS = [
  ...
  { id: 'bulk', title: 'Bulk Services', description: 'Industry-leading bulk email and SMS marketing infrastructure.', icon: Mails, href: '/bulk-email' },
  { id: 'smtp', title: 'SMTP Server', description: 'Dedicated, professional SMTP servers for high deliverability.', icon: Server, href: '/smtp-server' },
];

const TABS_DATA = [
  { id: 'smtp', label: 'SMTP Server', title: 'Precision SMTP Routing', Visual: SmtpIllustration, description: 'Bypass standard limits with our dedicated SMTP servers. Process thousands of emails efficiently, attract new clients, and maintain critical relationships with zero latency and maximum uptime.' },
  { id: 'bulk-email', label: 'Bulk Email', title: 'Inbox-First Deliverability', Visual: BulkEmailIllustration, description: 'Scale your outreach with our high-deliverability bulk email infrastructure. Automated warm-up protocols and IP pooling ensure your campaigns bypass spam filters reliably.' },
  { id: 'bulk-sms', label: 'SMS Gateway', title: 'Enterprise SMS Gateway', Visual: SmsIllustration, description: 'Cut through the noise with a 98% open rate. Our global SMS gateway offers fast delivery and robust APIs to trigger transactional alerts or blast promotional campaigns instantly.' },
  { id: 'email-marketing', label: 'Marketing Suite', title: 'Intelligent Workflows', Visual: MarketingIllustration, description: 'Build campaigns with our intuitive editor, automate complex customer journeys, and track success with real-time analytics. Boost engagement with smart A/B testing.' },
];
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-12 grid grid-cols-1 gap-4 sm:mb-16 sm:grid-cols-3 sm:gap-6 lg:gap-8">
          {OVERVIEW_CARDS.map((card) => {
            ...
            return (
              <article key={card.id} className="group relative flex flex-col items-start justify-between rounded-2xl border border-slate-200 bg-white p-5 transition-all hover:border-blue-200 hover:shadow-[0_8px_30px_rgb(0,0,0,0.04)] sm:rounded-3xl sm:p-7">
                ...
                <a href={card.href} className="inline-flex items-center gap-1.5 text-xs font-semibold text-slate-900 transition-colors hover:text-blue-600 sm:text-sm">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 lg:divide-x lg:divide-slate-200">
            ...
            <div className="flex flex-col p-4 sm:p-8 lg:col-span-8 lg:p-10">
              ...
              <div key={activeData.id} className="motion-safe:animate-in motion-safe:fade-in motion-safe:slide-in-from-bottom-2 motion-safe:duration-500">
                ...
                <button className="rounded-lg bg-slate-900 px-5 py-2.5 text-xs font-semibold text-white transition-all hover:-translate-y-0.5 hover:bg-blue-600 hover:shadow-lg hover:shadow-blue-600/25 sm:rounded-xl sm:px-6 sm:py-3 sm:text-sm">
